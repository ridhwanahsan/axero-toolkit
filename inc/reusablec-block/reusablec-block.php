...
if (defined('WPB_VC_VERSION')) {
    ...
    if (function_exists('vc_set_default_editor_post_types')) {
        vc_set_default_editor_post_types(['page', 'axero_blocks']);
        //For The7
...
function rcb_get_content_func($atts)
{
    ...
    if (absint($pagid)) {
        ...
        if (get_the_ID() == $pagid) {
            return __('Oops! You are trying to display contnet of this page in this page itself using shortcode?', 'axero-toolkit');
        }
        ...
        return __('Invlaied ID', 'axero-toolkit');
    }
}

//register shortcode
add_shortcode('axero_block', 'rcb_get_content_func');

//fucntion for template call
function axero_block_by_id($id)
{
    echo rcb_get_content_func(['id' => $id]);
}
