...
function rcb_register_options_page() {
	add_submenu_page( 'edit.php?post_type=lunex_blocks', 'Reusable Content Blocks Settings', 'Settings', 'manage_options', 'reusablecb-options', 'rcb_options_page_callback' ); 
}
...
function rcb_options_page_callback() {
	?>
	<div class="wrap">
	<h2><?php echo esc_html__( 'Reusable Content Blocks Settings', 'lunex-toolkit' ); ?></h2>	
    ...
    <table class="form-table">
        ...
        <tr valign="top">
        <th scope="row"><?php echo esc_html__( 'Hide unpublished blocks:', 'lunex-toolkit' ); ?></th>
        <td><input type="checkbox" name="rcb_hide_unpub" value="enable" <?php checked( get_option('rcb_hide_unpub'), 'enable', true ); ?>/></td>
        <td></td>
        </tr>
        <tr valign="top">
        <th scope="row"><?php echo esc_html__( 'Enable widget support:', 'lunex-toolkit' ); ?></th>
        <td><input type="checkbox" name="rcb_enable_widget" value="enable" <?php checked( get_option('rcb_enable_widget'), 'enable', true ); ?>/></td>
        <td><?php _e( 'If your Theme support shortcodes in Widget areas, you can use reusable shortcodes in Text/HTML Widgets.', 'lunex-toolkit' ); ?>
        </td>
        </tr>
        
        <tr valign="top">
        <th scope="row"><?php echo esc_html__( 'CSS Metakeys:', 'lunex-toolkit' ); ?></th>
        <td><textarea name="rcb_css_metakeys" rows="5" cols="40"><?php echo get_option('rcb_css_metakeys'); ?></textarea></td>
        <td><?php echo sprintf( __( '(Optional) Some page builders save element\'s custom CSS as meta keys and render it when the page is viewed. If elements from your theme or page builder addons missing style when inserted with shortcode, add your meta keys here one in a line.', 'lunex-toolkit' ) );  ?>
        </td>
        </tr>
    </table>
    <?php submit_button();  ?>
