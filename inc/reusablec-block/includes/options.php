<?php
    ...
    {
        add_submenu_page('edit.php?post_type=axero_blocks', 'Reusable Content Blocks Settings', 'Settings', 'manage_options', 'reusablecb-options', 'rcb_options_page_callback');
    }
    ...
    ?>
	<div class="wrap">
	<h2><?php echo esc_html__('Reusable Content Blocks Settings', 'axero-toolkit'); ?></h2>
...
<?php do_settings_sections('rcb-settings-group'); ?>
    ...
    <table class="form-table">
        ...
        <tr valign="top">
        <th scope="row"><?php echo esc_html__('Hide unpublished blocks:', 'axero-toolkit'); ?></th>
        <td><input type="checkbox" name="rcb_hide_unpub" value="enable"                                                                                                                                               <?php checked(get_option('rcb_hide_unpub'), 'enable', true); ?>/></td>
        <td></td>
        </tr>
        <tr valign="top">
        <th scope="row"><?php echo esc_html__('Enable widget support:', 'axero-toolkit'); ?></th>
        <td><input type="checkbox" name="rcb_enable_widget" value="enable"                                                                                                                                                     <?php checked(get_option('rcb_enable_widget'), 'enable', true); ?>/></td>
        <td><?php _e('If your Theme support shortcodes in Widget areas, you can use reusable shortcodes in Text/HTML Widgets.', 'axero-toolkit'); ?>
        </td>
        </tr>

        <tr valign="top">
        <th scope="row"><?php echo esc_html__('CSS Metakeys:', 'axero-toolkit'); ?></th>
        <td><textarea name="rcb_css_metakeys" rows="5" cols="40"><?php echo get_option('rcb_css_metakeys'); ?></textarea></td>
        <td><?php echo sprintf(__('(Optional) Some page builders save element\'s custom CSS as meta keys and render it when the page is viewed. If elements from your theme or page builder addons missing style when inserted with shortcode, add your meta keys here one in a line.', 'axero-toolkit')); ?>
        </td>
        </tr>
    </table>
    <?php submit_button(); ?>
