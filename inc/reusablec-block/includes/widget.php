<?php
     ...
     *
     * Provides a widget for inserting contnet into a widget area. can enable/disable from Axero Blocks > Settings
     **/
    ...
    class Reusable_Block extends WP_Widget
    {
        ...
        public function __construct()
        {
            ...
            add_action('sidebar_admin_page', [$this, 'rcb_widget_admin_js']);

            // Load admin javascript for Widget options on theme customize page (customize.php).
            add_action('customize_controls_enqueue_scripts', [$this, 'rcb_widget_admin_js']);
        ...
        {
            //load widget script
            wp_enqueue_script('reusablecb-widget', plugin_dir_url(__FILE__) . 'js/reusablecb-widget.js', ['jquery']);
        }
        ...
        <label for="<?php echo esc_attr($this->get_field_id('id')); ?>"><?php echo esc_html__('Select Block:', 'axero-toolkit'); ?></label>
		<select
            class="widefat"
		...
		<?php echo esc_html__('Enter post ID:', 'axero-toolkit'); ?></label><br />
        <input
		...
		<?php
                ...
                public function widget($args, $instance)
                {
                    ...
                    if (! empty($instance['id']) || ! empty($instance['othe_id'])) {
                        ...
                        echo esc_html__('No content selected!', 'axero-toolkit');
                    }
